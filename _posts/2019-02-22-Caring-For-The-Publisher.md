...
Here at Adello, not only that we care how our creatives impact the publisher's overall page performance, we are also very aware that what helps the publisher, helps us, and two things would be load speed and seamless integration.
...
### Vanilla Code
It is tempting to jump over using online libraries to get things done quicker and easier, such as jQuery, Bootstrap, GreenSock to name a few, but if they are not heavily used, such as in a full-featured website, then they add too much overhead and impact load and browsing performance.
Taking jQuery as example, nowadays, JavaScript is better supported in most browsers and we no longer have to cover so much cross-browser compatibility solutions. Already, a wide number of jQuery methods are supported natively by JavaScript and I don't need to say this, but native is better, and faster!
...
I will stress a bit more that jQuery and other similar libraries and frameworks still have strong uses, though creatives are lighter in their nature and we need to weigh the Use versus Overhead.

### Naming Convention and Code Structure
Our creatives get loaded in various environments, in friendly or non-friendly iframes, inside native apps , but a significant amount of times, a creative is also loaded directly in the publisher's topframe, which means our styling and javascript has direct access to the publisher's DOM, and vice versa.
...
Though when styling our creatives, we have to be careful not create global selectors that can target the publisher's elements as well, so we tie all of our styles to a main namespace, as illustrated below:
...
In JavaScript we have a similar situation, where sometimes methods and functionality is added directly onto the global namespace and this can cause collision with the publisher's variables or methods.
...
Another easy reach for simplicity is a CSS boilerplate to either normalize or stylize html elements so that we no longer have to write basic styles, but since a creative is very light in its nature when it comes to its mark-up language, even a simple boilerplate can be overhead, ending up with normalization of elements you don't use or having to rewrite the normalization styles anyway.
...
### Code minification
Code minification goes without saying, the final code that gets delivered is no longer needed to be human readable, therefore we can apply code minification to remove comments, spaces, new lines and other unnecessary code for the machine. Most importantly, this can also be applied for css and html, where unnecessary code can be removed. In general, the minification can reduce up to 60% and that is a significant change in the creative's load time.

### Loading resources async over sync
Scripts should be loaded async, otherwise they block rendering and increase the publisher page's overall loading time. If we need to write javascript that depends on a script, the solution is to simply use the onload callback to run JavaScript after the dependency was loaded, this allowing the script to be parsed in parallel with other resources.
...
Ever swiped a page to scroll but it simply lagged or stuttered? That is usually Scroll Jank and it is caused by event listeners in the page. An event listener, such as touchstart or touchmove can block scrolling, so the browser instead of scrolling smoothly, it has to check each event listener to make sure that it is not trying to block the scrolling via event.preventDefault, losing precious time and performance doing so. The problem comes when there are such event listeners that don't block scrolling, but the browser has to check then anyway. And here is where Passive Event listeners come in, by being able to promise the browser that you will not call event.preventDefault() and the browser no longer has to check and can continue the scroll smoothly.
...
While they have been here for quite a while, they are used in the 3rd parameter for an event listener, which means that for older browser that do not support them, the above will translate into an event listener with Capture Event enabled, since the object {passive: true} will be read as a truthy value. But in order to ensure you can use it safely in older browsers, a detection for Passive Events Listeners can be used before attaching them:
...
The Passive Event listeners were mainly created for mobile and it is a game changer for 3rd party content as well, which is why we use them, to ensure that we do not impact the scrolling performance for the publishers that run our creatives.
...
Compression
There are more modern image formats, such as WebP and JP2 (JPG 2000), that have a superior compression and better quality characteristics than their counterparts, JPG and PNG.
